...
| # | Sev | Disposition |
|---|---|---|
| **H7** | blocker | **Closed, by deciding it was never an issue.** `new-app/db/migrations/20260926120000_rejected_seals/migration.sql` gives a refused seal its own append-only `rejected_seal` table with its own true `sealed_at`, so none of the three impossible routes is needed; `docs/PRD.md` R1.18c and R1.18g-i and `docs/design/domain-model.md` §8 state it, including that first-to-sync decides while both timestamps are kept so who priced it first stays answerable. Eight tests, three plants, control green |
| **H12** | major | **Closed by removing the queue.** `docs/PRD.md` R1.32b-c and `docs/design/domain-model.md` §8: nothing numbers a blocked seal automatically, so the finding's three questions dissolve — the tenant numbers it, in whatever order they choose, against the month they do it in (ADR 0023). Automatic FIFO would spend a scarce monthly allowance on whichever job happened to be sealed first, possibly one the contractor has abandoned, which is a commitment the product should not make for them. A blocked seal does not expire, and upgrading lifts the limit rather than releasing the seals |
| **H9** | blocker | **Closed by design, approved 2026-09-26.** `docs/design/acceptance-evidence.md` requires a channel at the **point of use** rather than on the client row — a walk-up at a gate has neither, and a share link may only be minted for a client with at least one, typed at send time if need be. `docs/PRD.md` R1.20e and `docs/design/domain-model.md`'s `client` row carry it; `docs/TIERS.md` §3 points at the ladder. `new-app/web/content/site.ts` checked and already correct — it says the client "confirms with a code sent to them", which is grade 3. **`new-app/web/test/site-guards.test.ts` does not cover this** and the limit is stated rather than implied: no guard asserts that a described flow is the one built |
| **H10** | major | **Closed by naming what cannot be fixed.** `docs/design/acceptance-evidence.md` states first that no mechanism where the tenant supplies the counterparty's address proves anything against the tenant, then uses the only two things that help — a third party the tenant does not control (money, an inbound reply, a signature provider) and honest labelling. Six grades, derived from append-only evidence, which `docs/design/domain-model.md` now carries as the `acceptance_evidence` row with the grade explicitly derived rather than stored. **`docs/THREAT-MODEL.md` §4c is new** and records it as a structural limit rather than a bug, including the sentence the UI must not say. `docs/PRD.md` R1.20/R1.20f-h and `docs/SERVICE-REGISTER.md` §3b (inbound handling, owed) |
| **H1** | blocker | **Closed.** `docs/design/domain-model.md` §6.2a and the `invoice` row now say **recorded** variations and deliberately do not restate the arithmetic — it lives once in `issue_ceiling_minor()`, since restating it is how the two documents came to disagree twice (G1, then H1). `docs/PRD.md` R1.24/R1.24d already said recorded |
| **H2** | blocker | **Closed again 2026-09-26, after J12 reopened it.** The first disposition claimed `docs/design/domain-model.md` §6.2a "no longer carries a prose writer list"; it did, as the table's third column two lines above the sentence retiring it, and that column credited a credit note as a writer of `invoiced_total_minor` — which it has never been. §6.2a now carries **no writer column at all**, and the two questions it used to answer in prose have executed homes: `new-app/db/test/documents-core.test.ts` (the J12 block) asserts which insert moves which column, including that a credit note moves none, and `new-app/db/migrations/20260926130000_ceiling_enforced_by_trigger/migration.sql` is what makes the call happen. `docs/PRD.md` R1.24a-b are unchanged and still correct; policy-parity asserts the flag predicate |
| **H3** | blocker | **Closed.** `docs/PRD.md` R1.22b now says recording a variation re-derives the **ceiling**, not the accepted total, which is written once (R1.24b); `docs/design/domain-model.md` §6.2a's column table already distinguished them |
| **H5** | blocker | **Closed.** `docs/design/domain-model.md` §6.3 reframes it: superseding an accepted issue is not a forbidden transition but an ordering — withdraw first, which H4's trigger permits only while no invoice or variation exists, and the issue is then not accepted so nothing is orphaned. `docs/PRD.md` R1.15/R1.15b-c state the same |
| **H6** | blocker | **Closed by deletion.** `docs/design/domain-model.md` had two state machines for one thing in one file; §8's diagram is gone and §6.3 is a table of what each state *means*, with `quote_issue_state()` named as the definition. There is no state column, so there is no value for two paragraphs to disagree about. `docs/PRD.md` R1.18 unchanged and still correct |
| **H8** | blocker | **Closed.** `docs/design/domain-model.md` names `registration_claim` beside the `user` row and explains how they interlock: the claim holds nothing, verification inserts the user, the existing global unique index decides. `docs/PRD.md` R1.30b-e and `docs/RULES.md` Rule 14 agree; `docs/TIERS.md` §2a checked and already consistent — it lists what sign-up needs and names nothing this contradicts |
| **H13** | major | **Closed.** `docs/PRD.md` R1.30a now quotes `docs/RULES.md` Rule 14 **as it currently reads** — "one tenant per verified address" — rather than wording the same commit had deleted; `docs/TIERS.md` §2a unchanged and consistent |
| **H14** | major | **Closed.** `docs/PRD.md` R1.30d restores the **per-address** rate limit its own exhaustive list had dropped; `docs/RULES.md` Rule 14 requires it; **`docs/THREAT-MODEL.md` §4b is new** and names the consequence the finding identified — registration as an email bomb aimed at a known address, because the non-enumerating duplicate response mails the existing owner, so the two must ship together; `docs/TIERS.md` §2a checked and consistent |
| **H15** | major | **Closed.** `docs/TIERS.md` §3 item 3 records that ADR 0024 and finding F17 overtook both halves of what it prescribed, and its ladder now marks the three release-2 features to match `new-app/web/content/site.ts`. `docs/PRD.md` R1.20-R1.20d and `docs/design/domain-model.md`'s `acceptance` row were checked and already correct — the code-verified signature and the single `document_render` hash. The need the item names is unchanged |
| **H18** | minor | **Closed.** `docs/PRD.md` §7 meters jobs **numbered** rather than "quoted", and the Free offline row carries the caveat that a seal past the limit waits; `docs/TIERS.md` and `new-app/web/content/site.ts` already said numbered. **`new-app/web/test/site-guards.test.ts` does NOT cover this** — it checks that a tier line is delivered or marked, not that a caveat is present — and that limit is stated rather than papered over |
