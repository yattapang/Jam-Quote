...
| **H10** | major | **Closed by naming what cannot be fixed.** `docs/design/acceptance-evidence.md` states first that no mechanism where the tenant supplies the counterparty's address proves anything against the tenant, then uses the only two things that help — a third party the tenant does not control (money, an inbound reply, a signature provider) and honest labelling. Six grades, derived from append-only evidence. **`docs/THREAT-MODEL.md` §4c is new** and records it as a structural limit rather than a bug, including the sentence the UI must not say. `docs/PRD.md` R1.20/R1.20f-h and `docs/SERVICE-REGISTER.md` §3b (inbound handling, owed) |
