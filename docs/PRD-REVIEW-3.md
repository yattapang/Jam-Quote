...
| **H2** | blocker | **Closed.** `docs/design/domain-model.md` §6.2a no longer carries a prose writer list — a list that can be wrong is not a control. `issue_balance` has no write policy the application can satisfy, so the writer set IS the set of callers of `issue_balance_open`/`issue_balance_apply` and cannot go stale; `docs/PRD.md` R1.24a-b state the same and policy-parity asserts the flag predicate |
