...
| [0025](0025-five-invariants-move-from-prose-to-code.md) | Three review passes each introduced what the next found, so five invariants leave prose for code: the ceiling says "recorded" once; `issue_balance` has no write grant and one locked function; a document's state is derived, never stored; withdrawal is an insert-only row; a pending registration is not a user | Accepted |
