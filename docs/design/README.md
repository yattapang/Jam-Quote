...
| Design | Status | For |
|---|---|---|
| [acceptance-evidence.md](acceptance-evidence.md) | **Approved** 2026-09-26, review outstanding | How a client accepts and what it is worth (findings H9, H10). A ladder of six grades with the grade **derived** from append-only evidence rows, a channel required at the point of use rather than on the client row, and the tenant choosing the bar per quote. States first what cannot be fixed: no mechanism where the tenant supplies the counterparty's address proves anything against the tenant. Release 1 cannot have the client's own reply, so the deposit is the strongest grade available |
