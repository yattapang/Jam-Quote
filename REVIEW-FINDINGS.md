...
|---|---|
| "139 string fields, 0 unbounded" | True of the API DTOs, which is all the guard scans. The quote and invoice **line item** schemas live in `packages/core`, and `description` and `overrideNote` carry no upper bound — on every line of every document. |
| The markup leak is closed by explicit selects **plus** strict contracts | The select is the fix. The contract validates a hand-written sample, while the service assigns the line items straight from a Prisma result, so TypeScript's excess-property check never runs. Add `markupPct` to `PUBLIC_LINE_SELECT` and everything compiles and every test passes. **Fix: derive the sample from the select via `Prisma.QuoteLineItemGetPayload`.** The tenancy reviewer confirmed the *view-level* double coupling does hold; it is the line select that is unguarded. |
...
| `hand-written-shapes.test.ts` stops the list growing | It catches an exported interface and misses an exported type alias. And roughly fifteen hand-written response shapes in the same file do not start with `Api` — including `AdminFinancials` with `mrrCents`, `PricingConfig`, and `EffectiveRulePack` with its tax percentages. |
...
| `admin-console-honesty.test.ts` guards against fabricated figures | It is five lists of historical strings plus a naming-convention check. Its own docblock claims a "recognisable placeholder" check that does not exist. It passes green alongside F17, F20 and F38, and no component suite renders that screen at all. |
