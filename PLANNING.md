...
| 3 | **Advisory lock / migrations** | **Fixed at the cause `a7bb66c`.** `directUrl` + `DIRECT_URL` on Render means migrations run unpooled, so the session-level lock releases instead of being recycled by pgbouncer with the lock held. **`PRISMA_SCHEMA_DISABLE_ADVISORY_LOCK=1` must STAY** until the already-stranded session (pid 6792, idle pgbouncer) is gone — removing it today breaks the next deploy. It clears when Neon suspends the idle compute, or on a manual restart; drop the flag after that and re-test. Verified: directUrl + flag = "No pending migrations to apply." |
