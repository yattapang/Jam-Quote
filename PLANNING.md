...
| 2 | **"Couldn't load 1 section"** | Migrations now applied (`rulepack_maintainable`, `subscription_interval`), which should clear it if rule-pack was the failing section. **Unconfirmed — the banner names the path and I have not seen it.** The banner now also warns explicitly when `/admin/me` is the failure, because that one silently hides every management control. |
...
| 4 | **Tenant management options still not visible** | Owner has 3 real tenants and no suspend/delete/plan controls. The controls render only when `id && canManageTenants`; ids are real now, so `canManageTenants` is false — meaning `/admin/me` returned no capabilities, which `safe()` produces on ANY failure. **Need the banner's exact text to confirm.** |
