...
| Brand (2026-09-20) | The product is Pryvis (pryvis.com). Rename sequenced: user-visible copy now; the `@jamquote/*` npm scope and the repo after the first feature ships; database identifiers are not renamed. ADR 0009. |
| Tiers (2026-09-20) | Free, Pro, Business. Free = quoting (3 jobs/month) and sharing; Pro adds invoicing, payments, reminders, card links, costing, exports and offline mobile; Business adds team seats (10 then per seat), roles and approvals, crews, consolidated reporting, WhatsApp Business sending and API access. `docs/TIERS.md`. |
| Entitlements (2026-09-20) | Named entitlements resolved server-side, never a `plan` comparison at a call site; tier ladder in core, prices per country in `PlanTierConfig`; existing tenants grandfathered by explicit audited grants. ADR 0007. |
| Tier pricing (2026-09-20) | Ship the entitlement machinery first carrying today's Pro price forward; set Pro and Business prices per country afterwards as an admin change. |
| Release process (2026-09-20) | One feature at a time: design, build, verify, launch, then the next. Tiers and entitlements first, so later features plug into them instead of being retrofitted. `docs/BUILD-RULES.md` section 8. |
| Module completion (2026-09-20) | A module is complete only after an independent attack pass by someone who did not build it, and every dependency between modules has a seam test against a real database, proved by planting a defect on one side. `docs/BUILD-RULES.md` section 9; register in `docs/MODULE-SEAMS.md`. |
| Security (2026-09-20) | Security is built into every module from its first commit: default-deny authorisation, tenant scope from the token only, schema validation at the boundary, projected output, secrets in the environment, plain errors, rate limits on, every money/permission/tenancy change audited, hostile-until-proven uploads, reviewed dependencies and migrations. Each module ships negative tests and a security attack pass. `docs/BUILD-RULES.md` section 10. |
...
| **Deploy API and web TOGETHER** — now MANDATORY, not advisory | Field renames, moved routes, and `issueDate` now required on the reports invoice type. Mismatched halves fail requests rather than degrading. **And a contract that LOSES a field is a hard break in both directions** — `unitPriceCents` became `amountCents` on the public line views, so a skewed deploy prints `NaN` on a client's share page. See `packages/core/src/wire/README.md` for the rule. |
...
| What does voiding a payment BUY the tenant? | When a payment is voided, a surviving one slides back onto the emptied period — so the tenant's cover can end on the day they paid, reading PAST_DUE immediately. That is the "money buys the earliest unpaid month" rule adopted earlier, and it is defensible; the alternative is that money buys cover from when it arrived and voiding only shortens the tail. A review called the current behaviour a bug; simulation showed every candidate rule agrees there, so it is a POLICY choice, not a defect |
